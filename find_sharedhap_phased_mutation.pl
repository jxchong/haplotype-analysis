...
my ($prevsubjectid, $prev_max, $prev_nzeros, $prevstart, $prevstop) = ((0)x5);
...
while ( <FILE> ) {
	...
	for (my $m=0; $m<=$maxmismatch; $m++) {
		...
		for (my $pos=$mutationpos_left; $pos>=0; $pos--) {
			if ($refhap[$pos] eq $currenthaplotype[$pos] || $refhap[$pos] eq '0') {
				$nsnps_match++;
				$endmatchpos = $pos;
			} elsif ($currenthaplotype[$pos] eq '0') {
				$nsnps_match++;
				$nzeros++;
				$endmatchpos = $pos;
			} else {
	...
	if ($prevsubjectid == $subjectid) {	
		my $matchfound = 0;
		$countanalyzedsubj++;
		if ($prev_max > $minsnpstomatch) {  # $prev_max > $maxlength && 
			if ($maxsnps > $minsnpstomatch) {
				print "+";
			}
			print join("\t", @previnfo)."\n";
			$matchfound = 1;
	...
	$prev_max = $maxsnps;
	$prevstart = $maxmatch_start_snpno;
	$prevstop = $maxmatch_end_snpno;
